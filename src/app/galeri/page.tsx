...
// Buraya ileride kendi operasyon, araç ve depo fotoğraflarınızın linklerini/yollarını ekleyeceksiniz
const galleryImages = [
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24 pb-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12 text-center">
        <Reveal direction="up">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-orange-50 text-brand-accent font-semibold text-sm mb-4 border border-orange-100">
            <Camera size={18} /> Sahadan Kareler
          </div>
          <h1 className="text-4xl md:text-5xl font-extrabold text-brand-dark tracking-tight mb-4">
            Fotoğraf <span className="text-brand-accent">Galerisi</span>
          </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {galleryImages.map((src, index) => (
            <Reveal key={index} direction="up" delay={index * 0.1}>
              <div 
                className="group relative w-full h-64 md:h-80 rounded-2xl overflow-hidden cursor-pointer shadow-md hover:shadow-2xl transition-all duration-300"
              ...
              >
                ...
                />
                {/* Üzerine gelince çıkan karartma ve ikon */}
                <div className="absolute inset-0 bg-brand-dark/0 group-hover:bg-brand-dark/40 transition-colors duration-300 flex items-center justify-center">
                  <div className="bg-brand-accent text-white p-3 rounded-full opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                    <Maximize2 size={24} />
                  </div>
                </div>
              </div>
